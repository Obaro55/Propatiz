...
<?php
/**
 * Template Name: Dashboard - Submit Listing
 *
 */
...
$propatiz_plan = get_user_meta($userID, 'propatiz_plan', true);

if($propatiz_plan == 'regular') {
wp_redirect(esc_url(home_url('/choose-a-plan/')));
exit();
}
...
$youtube_links = array_map('esc_attr', $youtube_links);
    ...
    $cities = array("Ogba", "Ifako-Ijaiye", "Omole", "Ojodu", "Agidingbi", "Alausa", "Magodo", "Isheri", "Ikeja", "Dolphin Estate", "Lagos Island", "Ikoyi", "Lekki", "Victoria Island", "Surulere", "Maryland", "Ilupeju", "Festac", "Satellite Town", "Iju", "Agege", "Iyana Ipaja", "Igando", "Oshodi", "Isolo", "Epe", "Ikorodu", "Badagry", "Sagamu", "Abeokuta", "Ijebu Ode", "Ibadan", "Bodija", "Ogbomosho", "Ife", "Ile Ife", "Ilesha", "Ilesa", "Ado Ekiti", "Akure", "Ondo City", "Ilorin", "Benin", "Benin City", "Warri", "Sapele", "Asaba", "Agbor", "Yenagoa", "Awka", "Onitsha", "Owerri", "Aba", "Umuahia", "Nsukka", "Abakaliki", "Port Harcourt", "Uyo", "Calabar", "Lokoja", "Okene", "Makurdi", "Abuja", "Kubwa", "Garki", "Maitama", "Jos", "Zaria", "Bida", "Gusau", "Potiskum", "Minna", "Lavun", "Suleja", "Maiduguri" , "Mubi", "Yola");
    
    
    if(in_array($property_city, $cities) && $classification == "Properties for Sale" && !empty($property_state)) {
    ...
    $extra_classification = $property_type . " for Sale in " . $property_city;
	...
	if(in_array($property_city, $cities) && $classification == "Properties for Rent" && !empty($property_state)) {
    ...
    $extra_classification = $property_type . " for Rent in " . $property_city;
	...
	if(in_array($property_city, $cities) && $classification == "Properties for Lease" && !empty($property_state)) {
    ...
    $extra_classification = $property_type . " for Lease in " . $property_city;
	...
	update_post_meta($post_id, 'nearby_places', $nearby_places);
	}
	...
	update_post_meta($post_id, 'property_listing_status', 'regular');
	
	$propatiz_plan = get_user_meta($userID, 'propatiz_plan', true);
	

	if($propatiz_plan == 'one_listing_plan' || $propatiz_plan == 'three_listings_plan') {

    $listing_count = get_user_meta($userID, 'propatiz_listing_count', true);
	$listing_count = intval($listing_count)-1;
	
	update_user_meta($userID, 'propatiz_listing_count', $listing_count);

	}
	
	$propatiz_listing_count = get_user_meta($userID, 'propatiz_listing_count', true);
	
	if($propatiz_plan == 'one_listing_plan' || $propatiz_plan == 'three_listings_plan' && $propatiz_listing_count == 0) {
	    
	update_user_meta($userID, 'propatiz_plan', 'regular');   
...
<div id="primary" class="content-area">
	<main id="main" class="site-main dashboard" role="main">
		...
		<div class="dashboard listing-guidelines-header">
		<h1>Kindly ensure you read our listing <a href="https://www.propatiz.com/listing-guidelines/">submission guidelines</a> before continuining.</h1>
		</div>
		...
		<div class="listing-submission-form">
			...
			<option value="Properties for Sale">For Sale</option>
			<option value="Properties for Rent">For Rent</option>
			<option value="Properties for Lease">For Lease</option>
			</select>
			...
			<h2 class="upload-pictures">Upload pictures <span class="explanation">(the first picture becomes the main picture)</span></h2>
